...
import { getSuperAdmin, useSuperAdminAuth } from "@aura/super-admin/superAdmin/superAdminAuth";
    ...
    <div className="min-h-screen flex bg-slate-950 text-slate-100" data-testid="super-admin-shell">
      ...
      <aside className="hidden lg:flex w-64 flex-col border-r border-white/5 bg-slate-950">
        <div className="p-6 border-b border-white/5">
          <p className="text-eyebrow text-brand-accent">Platform</p>
          <div className="mt-2 flex items-center gap-2.5">
            <span className="w-8 h-8 rounded-[10px] bg-gradient-to-br from-brand-primary to-brand-accent grid place-items-center">
          ...
          {NAV.map((n) => {
            ...
            return (
              <Link key={n.to} to={n.to}
                className={`mx-3 px-3 py-2 rounded-[10px] text-sm flex items-center gap-3 transition-all ${active ? "bg-brand-primary text-white" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}
