...
import { SUPER_ADMINS, mockSuperLogin } from "@aura/super-admin/superAdmin/superAdminAuth";
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2 bg-slate-950 text-slate-100" data-testid="super-login-page">
      ...
      <div className="hidden lg:flex relative overflow-hidden">
        ...
        <div className="absolute -top-32 -right-32 w-96 h-96 rounded-full bg-brand-primary/30 blur-3xl"></div>
        <div className="absolute -bottom-32 -left-32 w-96 h-96 rounded-full bg-brand-accent/20 blur-3xl"></div>
        <div className="relative z-10 flex flex-col justify-between p-14">
          <div className="flex items-center gap-3">
            <span className="w-10 h-10 rounded-[12px] bg-gradient-to-br from-brand-primary to-brand-accent grid place-items-center">
          ...
          <div>
            <p className="text-eyebrow text-brand-accent-hover">Platform status</p>
      ...
      <div className="flex items-center justify-center p-6 md:p-14">
        <div className="w-full max-w-md">
          <p className="text-eyebrow text-brand-accent">Restricted access</p>
          ...
          <form onSubmit={submit} className="mt-8 space-y-4">
            <div>
              ...
              <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="you@aurahotels.com" className="mt-1 w-full bg-white/5 border border-white/10 rounded-full px-4 py-3 text-sm text-white outline-none focus:border-brand-primary" data-testid="super-login-email" />
            </div>
            <div>
              <label className="text-[10px] tracking-widest uppercase text-slate-500">Password</label>
              <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="••••••••" className="mt-1 w-full bg-white/5 border border-white/10 rounded-full px-4 py-3 text-sm text-white outline-none focus:border-brand-primary" data-testid="super-login-password" />
            </div>
            {error && <p className="text-xs text-rose-400" data-testid="super-login-error">{error}</p>}
            <button type="submit" className="w-full py-3 rounded-full bg-brand-primary hover:bg-brand-primary-hover text-white text-sm font-medium shadow-[0_10px_28px_rgba(79,70,229,0.35)]" data-testid="super-login-submit">
              Sign in
            </button>
          </form>
