...
import { TEMPLATES } from "@aura/shared/tenants/tenantRegistry";
import BrandPreview from "@aura/super-admin/superAdmin/components/BrandPreview";
          ...
          return (
            <div key={s.key} className="flex-1 flex items-center gap-2 last:flex-none">
              <div className={`flex items-center gap-3 ${active ? "text-white" : done ? "text-emerald-400" : "text-slate-500"}`}>
                <span className={`w-9 h-9 rounded-full grid place-items-center flex-shrink-0 transition-all ${done ? "bg-emerald-500/20" : active ? "bg-brand-primary" : "bg-white/5"}`} data-testid={`step-${s.key}`}>
      ...
      <div className={`${step === 2 ? "max-w-6xl" : "max-w-3xl"} mx-auto`}>
        ...
        {step === 0 && (
          <div className="space-y-4" data-testid="provision-step-profile">
            <p className="text-eyebrow text-brand-accent">Step 1 of 4</p>
        ...
        {step === 1 && (
          <div className="space-y-4" data-testid="provision-step-domain">
            <p className="text-eyebrow text-brand-accent">Step 2 of 4</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
              {[
                ...
                <button
                  ...
                  className={`p-5 text-left rounded-[14px] border transition-all ${form.domainMode === o.k ? "border-brand-primary bg-brand-primary/10" : "border-white/10 bg-white/5 hover:bg-white/10"}`}
                  data-testid={`domain-${o.k}`}
                >
                  <div className="flex items-center justify-between">
                    <i className={`fa-solid fa-${o.i} text-brand-accent`}></i>
        ...
        {step === 2 && (
          <div className="space-y-4" data-testid="provision-step-brand">
            <p className="text-eyebrow text-brand-accent">Step 3 of 4</p>
              ...
              {TEMPLATES.map((k) => {
                ...
                return (
                  <button
                    ...
                    className={`p-5 text-left rounded-[14px] border transition-all ${active ? "border-brand-primary bg-brand-primary/10" : "border-white/10 bg-white/5 hover:bg-white/10"}`}
            ...
            <div className="pt-2">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => set({ tier: "pro" })} className={`px-4 py-2 rounded-full text-xs flex items-center gap-1.5 ${form.tier === "pro" ? "bg-gradient-to-r from-brand-accent to-brand-accent-hover text-slate-900" : "bg-white/5 text-slate-300 hover:bg-white/10"}`} data-testid="tier-pro">
        ...
        {step === 3 && (
          <div className="space-y-4" data-testid="provision-step-seed">
            <p className="text-eyebrow text-brand-accent">Step 4 of 4</p>
            ...
            <div className="mt-4 space-y-2">
              {[
                ...
                <label key={it.k} className="flex items-center gap-3 p-3 rounded-[10px] bg-white/5 hover:bg-white/10 cursor-pointer" data-testid={`seed-${it.k}`}>
                  <input type="checkbox" checked={form[it.k]} onChange={(e) => set({ [it.k]: e.target.checked })} className="w-4 h-4 accent-brand-primary" />
                  <i className={`fa-solid fa-${it.i} text-brand-accent text-xs w-4`}></i>
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
          ...
          {step < STEPS.length - 1 ? (
            <button onClick={() => setStep((s) => s + 1)} disabled={!canNext()} className="px-6 py-2.5 rounded-full bg-brand-primary hover:bg-brand-primary-hover disabled:opacity-40 text-white text-sm shadow-[0_10px_28px_rgba(79,70,229,0.35)] flex items-center gap-2" data-testid="provision-next">
...
const Field = ({ label, value, onChange, placeholder, testid, required, prefix, className = "" }) => (
  <div className={className}>
    ...
    <div className="mt-1 flex items-center bg-white/5 border border-white/10 rounded-full focus-within:border-brand-primary">
