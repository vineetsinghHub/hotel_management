  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-end md:items-center justify-center p-0 md:p-8 bg-slate-900/60 backdrop-blur-sm" data-testid="review-prompt">
      <div className="bg-white w-full max-w-lg rounded-t-[24px] md:rounded-[24px] p-8 shadow-[0_40px_100px_rgba(15,23,42,0.35)] relative">
        ...
        <p className="text-eyebrow text-brand-accent">How was your stay?</p>
        ...
        <div className="mt-5 flex items-center gap-2" data-testid="review-stars" onMouseLeave={() => setHover(0)} role="radiogroup" aria-label="Star rating">
          {[1,2,3,4,5].map((n) => (
            <button key={n} onMouseEnter={() => setHover(n)} onClick={() => setRating(n)} className={`w-11 h-11 grid place-items-center transition-transform ${(hover || rating) >= n ? "text-brand-accent scale-110" : "text-slate-300"} press-scale`} aria-label={`${n} star${n>1?"s":""}`} data-testid={`star-${n}`}>
        ...
        <div className="mt-5">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {tagOpts.map((t) => (
              <button key={t} onClick={() => toggleTag(t)} className={`px-3 py-1.5 rounded-full text-xs border transition-all ${tags.includes(t) ? "bg-brand-primary text-white border-brand-primary" : "bg-brand-surface text-slate-700 border-transparent hover:border-slate-200"}`} data-testid={`tag-${t.toLowerCase()}`}>{t}</button>
            ))}
          </div>
        </div>

        <textarea value={text} onChange={(e) => setText(e.target.value)} rows={4} placeholder="Tell us about your favourite moment (optional)…" className="mt-4 w-full bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" data-testid="review-text" />
        ...
        <div className="mt-6 flex items-center justify-end gap-3">
          ...
          <button onClick={submit} className="px-5 py-2.5 rounded-full bg-brand-primary hover:bg-brand-primary-hover text-white text-sm shadow-[0_10px_28px_rgba(79,70,229,0.28)]" data-testid="review-submit">
