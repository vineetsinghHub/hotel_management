...
import { menu } from "@aura/shared/data/mockData";
...
export default function Dining() {
  ...
  return (
    <div className="bg-brand-surface" data-testid="dining-page">
      <Navbar transparent />
      ...
      <section className="relative h-[80vh] overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-10 h-full flex flex-col justify-end pb-16">
          ...
          <p className="text-eyebrow text-brand-accent-hover">Three Restaurants · One Palace</p>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 bg-white rounded-[24px] border border-slate-200 overflow-hidden" data-testid="chef-recommendation">
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="p-8">
                <p className="text-eyebrow text-brand-accent">Chef&apos;s Recommendation</p>
                ...
                <div className="mt-6 flex items-center gap-3">
                  <span className="inline-flex items-center gap-2 text-xs px-3 py-1.5 bg-brand-surface rounded-full">
                    <i className="fa-solid fa-award text-brand-accent"></i>Michelin recognized
                  </span>
                  <span className="inline-flex items-center gap-2 text-xs px-3 py-1.5 bg-brand-surface rounded-full">
                    <i className="fa-solid fa-star text-brand-accent"></i>4.98 rating
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-1 bg-white rounded-[24px] border border-slate-200 p-6" data-testid="reserve-table-card">
            <p className="text-eyebrow text-brand-accent">Reserve a Table</p>
            ...
            <div className="mt-5 space-y-3">
              <div>
                ...
                <input value={date} onChange={(e) => setDate(e.target.value)} className="mt-2 w-full bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" />
              </div>
              <div>
                ...
                <div className="mt-2 grid grid-cols-4 gap-2">
                  {["18:30", "19:00", "19:30", "20:00", "20:30", "21:00", "21:30", "22:00"].map((t) => (
                    <button key={t} onClick={() => setTime(t)}
                      className={`py-2.5 rounded-[10px] text-xs font-mono ${time === t ? "bg-brand-primary text-white" : "bg-brand-surface text-slate-700 hover:bg-slate-100"}`}>
                      {t}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="mt-2 flex items-center justify-between p-3 bg-brand-surface rounded-[14px]">
      ...
      <section className="py-20 px-6 md:px-10 bg-white border-y border-slate-200" data-testid="menu-section">
        <div className="max-w-6xl mx-auto">
          <div className="text-center">
            <p className="text-eyebrow text-brand-accent">The Menu</p>
          ...
          <div className="mt-10 flex items-center justify-center gap-2 flex-wrap">
            {cats.map((c) => (
              <button
                key={c}
                onClick={() => setCat(c)}
                className={`px-6 py-2.5 rounded-full text-sm transition-all ${cat === c ? "bg-slate-900 text-white" : "text-slate-700 hover:bg-brand-surface"}`}
      ...
      <section className="py-20 px-6 md:px-10">
        <div className="max-w-7xl mx-auto">
          <p className="text-eyebrow text-brand-accent">Gallery</p>
