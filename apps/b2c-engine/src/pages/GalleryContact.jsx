...
import { galleryImages, faqs, property } from "@aura/shared/data/mockData";
...
export default function GalleryContact() {
  ...
  return (
    <div className="bg-brand-surface" data-testid="gallery-page">
      <Navbar transparent />
        ...
        <section className="py-16 px-6 md:px-10">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white rounded-[24px] border border-slate-200 p-8">
              <p className="text-eyebrow text-brand-accent">Reach Us</p>
              ...
              <div className="mt-6 space-y-4 text-sm text-slate-700">
                ...
                <div className="flex items-center gap-4"><i className="fa-solid fa-phone text-brand-accent"></i><span className="font-mono">{property.phone}</span></div>
                <div className="flex items-center gap-4"><i className="fa-regular fa-envelope text-brand-accent"></i><span className="font-mono">{property.email}</span></div>
                <div className="flex items-start gap-4">
                  <i className="fa-solid fa-square-parking text-brand-accent mt-1"></i>
                  <div>
              ...
              <form onSubmit={(e) => e.preventDefault()} className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                <input placeholder="Full name" className="bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" />
                <input placeholder="Email" className="bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" />
                <textarea placeholder="How may we assist you?" rows={4} className="md:col-span-2 bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" />
                <button className="md:col-span-2 py-3 rounded-full bg-brand-primary hover:bg-brand-primary-hover text-white text-sm">Send Message</button>
              </form>
            </div>
            ...
            <div className="rounded-[24px] overflow-hidden border border-slate-200 relative min-h-[520px]">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                <div className="relative">
                  <span className="absolute inset-0 w-16 h-16 rounded-full bg-brand-primary/30 animate-ping"></span>
                  <div className="relative w-16 h-16 rounded-full bg-white grid place-items-center shadow-lg">
                    <i className="fa-solid fa-location-dot text-brand-primary text-xl"></i>
                  </div>
                </div>
              </div>
              {/* Nearby */}
              <div className="absolute bottom-6 left-6 right-6 bg-white/95 backdrop-blur-xl rounded-[18px] p-5">
                <p className="text-eyebrow text-brand-accent">Nearby Attractions</p>
      ...
      <section className="py-24 px-6 md:px-10 bg-white border-y border-slate-200" data-testid="faq-section">
        <div className="max-w-4xl mx-auto">
          <p className="text-eyebrow text-brand-accent text-center">Frequently Asked</p>
      ...
      <section className="py-24 px-6 md:px-10">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-eyebrow text-brand-accent">The Aura Journal</p>
