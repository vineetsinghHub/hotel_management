...
import { menuItems } from "@aura/shared/admin/adminMockData";
  ...
  return (
    <AdminLayout pageTitle="Restaurant · Menu">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6">
        ...
        {!readOnly && <button className="ml-auto text-xs bg-brand-primary text-white px-4 py-2 rounded-full" data-testid="new-dish-btn">+ New dish</button>}
      </div>
        ...
        {vis.map((m) => (
          <div key={m.id} className="p-5 bg-white rounded-[16px] border border-slate-200" data-testid={`menu-${m.id}`}>
            <div className="flex items-start justify-between">
              ...
              <button onClick={() => toggle(m.id)} disabled={readOnly} className={`w-10 h-5 rounded-full relative ${readOnly ? "opacity-50 cursor-not-allowed" : ""} ${m.active ? "bg-brand-primary" : "bg-slate-200"}`}><span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${m.active ? "left-[22px]" : "left-0.5"}`}></span></button>
            </div>
