...
import StaffShifts from "@aura/b2b-pms/admin/components/StaffShifts";
import { seedUsers } from "@aura/shared/admin/adminAuth";
import { ROLES, roleColor, roleLabel } from "@aura/shared/admin/roles";
  ...
  return (
    <AdminLayout pageTitle="Staff & Roles">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <p className="text-eyebrow text-brand-accent">Access Management</p>
          <h2 className="mt-1 font-serif text-2xl text-slate-900">Staff members ({users.length})</h2>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setBuilderOpen(true)} className="inline-flex items-center gap-2 border border-slate-200 hover:bg-slate-50 text-slate-800 text-sm px-5 py-2.5 rounded-full" data-testid="role-builder-btn"><i className="fa-solid fa-shield-halved text-[10px] text-brand-accent"></i>Create custom role</button>
          <button onClick={() => setAddOpen(true)} className="inline-flex items-center gap-2 bg-brand-primary hover:bg-brand-primary-hover text-white text-sm px-5 py-2.5 rounded-full" data-testid="add-staff-btn"><i className="fa-solid fa-plus text-[10px]"></i>Invite staff</button>
        </div>
      </div>
              ...
              <tr key={u.id} className="border-t border-slate-100 hover:bg-slate-50" data-testid={`staff-row-${u.id}`}>
                ...
                <td>
                  <select value={u.role} onChange={(e) => changeRole(u.id, e.target.value)}
                    className="bg-white border border-slate-200 rounded-full px-3 py-1.5 text-xs outline-none focus:border-brand-primary" data-testid={`role-select-${u.id}`}
      ...
      <div className="mt-8 bg-white rounded-[16px] border border-slate-200 p-6">
        <div className="flex items-center justify-between">
          <p className="text-eyebrow text-brand-accent">Role Reference</p>
          <p className="text-xs text-slate-500">{ROLES.length} built-in · {customRoles.length} custom</p>
        </div>
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
          {ROLES.map((r) => (
            <div key={r.key} className="p-4 rounded-[12px] border border-slate-100 bg-brand-surface">
          ...
          {customRoles.map((r) => (
            <div key={r.key} className="p-4 rounded-[12px] border border-brand-accent/40 bg-brand-accent/5" data-testid={`custom-role-${r.key}`}>
              <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: r.color }}></span><span className="text-sm text-slate-900 font-medium">{r.label}</span><span className="text-[9px] tracking-widest uppercase text-brand-accent ml-auto">Custom</span></div>
              <p className="mt-2 text-xs text-slate-500">{r.desc}</p>
      ...
      {addOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" data-testid="add-staff-modal">
          <div className="bg-white w-full max-w-md rounded-[20px] p-8 relative">
            ...
            <p className="text-eyebrow text-brand-accent">New staff</p>
            <h3 className="mt-1 font-serif text-2xl text-slate-900">Invite team member</h3>
            <div className="mt-5 space-y-4">
              <div><label className="text-eyebrow text-slate-500">Name</label><input value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} className="mt-2 w-full bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" data-testid="new-staff-name" /></div>
              <div><label className="text-eyebrow text-slate-500">Email</label><input value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} type="email" className="mt-2 w-full bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary font-mono" data-testid="new-staff-email" /></div>
              <div><label className="text-eyebrow text-slate-500">Title</label><input value={form.title} onChange={(e) => setForm({...form, title: e.target.value})} className="mt-2 w-full bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" data-testid="new-staff-title" /></div>
              <div>
                <label className="text-eyebrow text-slate-500">Role</label>
                <select value={form.role} onChange={(e) => setForm({...form, role: e.target.value})} className="mt-2 w-full bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" data-testid="new-staff-role">
                  {allRoles.map((r) => <option key={r.key} value={r.key}>{r.label}{r.custom ? " · custom" : ""}</option>)}
                </select>
              </div>
            </div>
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button onClick={addUser} className="px-5 py-2.5 rounded-full bg-brand-primary hover:bg-brand-primary-hover text-white text-sm" data-testid="new-staff-submit">Send invitation</button>
            </div>
          </div>
        </div>
      )}
                ...
                <div className="mt-3 grid grid-cols-1 gap-2">
                  {allRoles.map((r) => (
                    <button key={r.key} onClick={() => { changeRole(editUser.id, r.key); setEditUser({...editUser, role: r.key}); }}
                      className={`w-full text-left p-3 rounded-[12px] border flex items-center gap-3 ${editUser.role === r.key ? "border-brand-primary bg-indigo-50/40" : "border-slate-200 hover:border-slate-300"}`}>
                      <span className="w-2 h-2 rounded-full" style={{ backgroundColor: r.color }}></span>
                      <div className="flex-1"><p className="text-sm text-slate-900">{r.label}{r.custom && <span className="ml-2 text-[9px] tracking-widest uppercase text-brand-accent">Custom</span>}</p><p className="text-xs text-slate-500">{r.desc}</p></div>
                      {editUser.role === r.key && <i className="fa-solid fa-check text-brand-primary text-xs"></i>}
                    </button>
                  ))}
                </div>
              </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-8 bg-slate-900/60 backdrop-blur-sm" data-testid="role-builder-modal">
      <div className="bg-white w-full max-w-4xl max-h-[95vh] overflow-y-auto rounded-t-[24px] md:rounded-[24px] p-8 relative shadow-[0_40px_100px_rgba(15,23,42,0.35)]">
        ...
        <p className="text-eyebrow text-brand-accent">Custom Role</p>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Night Manager" className="mt-2 w-full bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" data-testid="role-name" />
          </div>
          <div>
            <label className="text-eyebrow text-slate-500">Description</label>
            <input value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="What this role can do..." className="mt-2 w-full bg-brand-surface border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-brand-primary" data-testid="role-desc" />
          </div>
        </div>
        ...
        <div className="mt-6 flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <button onClick={() => bulk("all-write")} className="px-3 py-1.5 rounded-full bg-brand-primary text-white" data-testid="bulk-write">Full access</button>
          </div>
        </div>
        ...
        <div className="mt-4 space-y-4">
          {MODULE_GROUPS.map((g) => (
            <div key={g.label} className="border border-slate-100 rounded-[14px] overflow-hidden">
              <div className="px-4 py-2 bg-brand-surface border-b border-slate-100">
                    ...
                    <div key={it.k} className="px-4 py-3 flex items-center justify-between" data-testid={`perm-row-${it.k}`}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`text-[10px] tracking-widest uppercase px-3 py-1 rounded-full border transition-all ${p.write ? "border-brand-primary bg-indigo-50 text-brand-primary" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}
        ...
        <div className="mt-6 flex items-center justify-between p-4 rounded-[14px] bg-brand-surface border border-slate-100" data-testid="role-summary">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={submit} className="text-xs px-5 py-2 rounded-full bg-brand-primary hover:bg-brand-primary-hover text-white" data-testid="role-save">Save role</button>
          </div>
        </div>
      </div>
    </div>
