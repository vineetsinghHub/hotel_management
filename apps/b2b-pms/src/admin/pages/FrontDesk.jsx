...
import { arrivals, statusColor } from "@aura/shared/admin/adminMockData";
    ...
    <AdminLayout pageTitle="Front Desk">
      ...
      <div className="bg-white rounded-[16px] border border-slate-200 overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center gap-4 justify-between">
          <div>
            <p className="text-eyebrow text-brand-accent">Today&apos;s desk</p>
          ...
          {TABS.map((t) => (
            <button
              ...
              className={`px-4 py-2.5 text-sm rounded-t-[10px] border-b-2 transition-colors whitespace-nowrap flex items-center gap-2 ${
                tab === t.k ? "border-brand-primary text-slate-900 font-medium" : "border-transparent text-slate-500 hover:text-slate-800"
              }`}
              data-testid={`fd-tab-${t.k}`}
            >
              <i className={`fa-solid fa-${t.icon} text-[11px]`}></i>
              {t.label}
              <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-full ${tab === t.k ? "bg-brand-accent/15 text-brand-accent" : "bg-slate-100 text-slate-500"}`}>{counts[t.k]}</span>
            </button>
          ))}
        </div>
              ...
              return (
                <tr key={a.id} className="border-t border-slate-100 hover:bg-slate-50" data-testid={`fd-row-${a.id}`}>
                  ...
                  <td className="px-5 text-right space-x-2">
                    {a.status === "confirmed" && (
                      <button onClick={() => check(a.id, "checked_in")} className="text-xs px-3 py-1.5 rounded-full bg-brand-primary text-white" data-testid={`checkin-${a.id}`}>Check in</button>
                    )}
                    {a.status === "checked_in" && (
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-8 bg-slate-900/60 backdrop-blur-sm" data-testid="checkout-modal">
      <div className="bg-white w-full max-w-3xl max-h-[92vh] overflow-y-auto rounded-t-[24px] md:rounded-[24px] p-8 shadow-[0_40px_100px_rgba(15,23,42,0.35)] relative">
        ...
        <p className="text-eyebrow text-brand-accent">Check-out</p>
        ...
        <div className="mt-6">
          ...
          <div className="border border-slate-100 rounded-[14px] divide-y divide-slate-100">
            <div className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3"><i className="fa-solid fa-bed text-brand-accent"></i><div><p className="text-sm text-slate-900">Room & taxes</p><p className="text-xs text-slate-500">{row.roomType} · 3 nights</p></div></div>
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setPayOpen(true)} disabled={cleared} className="text-xs text-brand-primary hover:underline disabled:text-slate-300 disabled:no-underline disabled:cursor-not-allowed" data-testid="add-payment-btn">+ Add payment</button>
          </div>
          <div className="border border-slate-100 rounded-[14px] divide-y divide-slate-100">
            {payments.map((p) => (
        ...
        <div className="mt-6 p-5 rounded-[16px] bg-brand-surface border border-slate-100 space-y-2 text-sm">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3 justify-end">
          ...
          {!cleared && <button onClick={payAll} className="px-5 py-2.5 rounded-full bg-brand-accent hover:bg-[#B08D1E] text-slate-900 text-sm font-medium" data-testid="settle-balance-btn"><i className="fa-solid fa-lock text-[10px] mr-1.5"></i>Settle ₹{balance.toLocaleString()} & continue</button>}
          <button
            onClick={onComplete}
            disabled={!cleared}
            className="px-6 py-2.5 rounded-full bg-brand-primary hover:bg-brand-primary-hover disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm"
        ...
        {payOpen && (
          <div className="mt-6 p-5 rounded-[14px] border border-slate-200 bg-white" data-testid="add-payment-form">
            <p className="text-eyebrow text-brand-accent">Add payment</p>
            <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="text-eyebrow text-slate-500">Method</label>
                <select value={payForm.method} onChange={(e) => setPayForm({...payForm, method: e.target.value})} className="mt-1 w-full bg-brand-surface border border-slate-200 rounded-[12px] px-3 py-2 text-sm outline-none focus:border-brand-primary" data-testid="pay-method">
              ...
              <div>
                ...
                <input type="number" value={payForm.amount || ""} onChange={(e) => setPayForm({...payForm, amount: parseInt(e.target.value || "0", 10)})} placeholder={balance.toString()} className="mt-1 w-full bg-brand-surface border border-slate-200 rounded-[12px] px-3 py-2 text-sm outline-none focus:border-brand-primary font-mono" data-testid="pay-amount" />
              </div>
            </div>
            <div className="mt-3 flex items-center gap-2 justify-end">
              <button onClick={() => setPayOpen(false)} className="text-xs px-4 py-2 rounded-full border border-slate-200">Cancel</button>
              <button onClick={addPayment} className="text-xs px-4 py-2 rounded-full bg-brand-primary text-white" data-testid="pay-submit">Charge</button>
            </div>
          </div>
        )}
      </div>
