...
import { reviews } from "@aura/shared/admin/adminMockData";
        ...
        {rows.map((r) => (
          <div key={r.id} className="p-6 bg-white rounded-[16px] border border-slate-200" data-testid={`rev-${r.id}`}>
            <div className="flex items-start justify-between">
              ...
              <div className="flex text-brand-accent">{Array.from({ length: 5 }).map((_, i) => <i key={i} className={`fa-solid fa-star text-xs ${i < r.rating ? "" : "text-slate-200"}`}></i>)}</div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
              ...
              {!r.responded && <button onClick={() => respond(r.id)} className="text-xs bg-brand-primary text-white px-4 py-2 rounded-full" data-testid={`respond-${r.id}`}>Respond</button>}
            </div>
          </div>
        ))}
      </div>
