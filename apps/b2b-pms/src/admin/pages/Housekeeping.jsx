...
import { housekeeping, hkAttendants, statusColor } from "@aura/shared/admin/adminMockData";
import FloorPlanBoard from "@aura/b2b-pms/admin/components/FloorPlanBoard";
  ...
  return (
    <AdminLayout pageTitle="Housekeeping">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-eyebrow text-brand-accent">Room status & assignments</p>
      ...
      {view === "table" ? (
              ...
              {rows.map((r) => { const st = statusColor(r.status); return (
                <tr key={r.id} className="border-t border-slate-100" data-testid={`hk-${r.id}`}>
                  ...
                  <td className="px-5 text-right space-x-2">
                    ...
                    <button onClick={() => setStatus(r.id, "inspected")} className="text-xs px-3 py-1.5 rounded-full bg-brand-primary text-white">Inspect</button>
                  </td>
                </tr>);
              })}
            </tbody>
        ...
        <DndContext sensors={sensors} onDragStart={(e) => setDragging(e.active.id)} onDragEnd={handleDragEnd} onDragCancel={() => setDragging(null)}>
          ...
          <p className="mt-6 text-xs text-slate-500"><i className="fa-solid fa-hand text-brand-accent mr-1.5"></i>Drag a room to reassign it to another attendant.</p>
        </DndContext>
      ) : (
        <FloorPlanBoard />
      )}
...
const AttendantColumn = ({ attendant, rooms }) => {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`bg-white rounded-[16px] border transition-all ${isOver ? "border-brand-primary shadow-[0_10px_28px_rgba(79,70,229,0.15)] ring-2 ring-brand-primary/10" : "border-slate-200"}`}
...
const RoomCard = ({ room, overlay = false }) => {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`p-3 rounded-[12px] border bg-white cursor-grab active:cursor-grabbing select-none transition-all ${overlay ? "border-brand-primary shadow-[0_20px_50px_rgba(79,70,229,0.30)] rotate-2" : isDragging ? "opacity-30 border-slate-200" : "border-slate-200 hover:border-slate-300 hover:shadow-sm"}`}
