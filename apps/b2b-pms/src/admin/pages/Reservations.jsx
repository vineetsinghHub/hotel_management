...
import KanbanReservations from "@aura/b2b-pms/admin/components/KanbanReservations";
import { arrivals, roomsInventory, statusColor } from "@aura/shared/admin/adminMockData";
    ...
    <AdminLayout pageTitle="Reservations">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {!readOnly && <button onClick={() => toast.success("New reservation form opened")} className="bg-brand-primary hover:bg-brand-primary-hover text-white text-xs px-4 py-2 rounded-full" data-testid="new-reservation-btn">+ Reservation</button>}
        </div>
      </div>
...
const GanttCalendar = ({ rows, anchor, onShift }) => {
  ...
  const statusBg = (s) => s === "checked_in" ? "bg-emerald-500/90 border-emerald-600 hover:bg-emerald-500" : s === "checked_out" ? "bg-slate-400 border-slate-500 hover:bg-slate-400/90" : "bg-brand-primary border-indigo-700 hover:bg-brand-primary-hover";
  ...
  return (
    <div className="bg-white rounded-[16px] border border-slate-200 overflow-hidden" data-testid="gantt-calendar">
      <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <p className="text-eyebrow text-brand-accent">Room-nights grid</p>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-max">
          ...
          {uniqueRooms.map((roomNum) => {
            ...
            return (
              <div key={roomNum} className="flex border-b border-slate-100 relative group hover:bg-brand-surface/60" data-testid={`gantt-row-${roomNum}`}>
                ...
                <div className="relative flex" style={{ width: CELL_W * DAYS }}>
                  {dates.map((d, i) => {
                    const isWeekend = d.getDay() === 0 || d.getDay() === 6;
                    return <div key={i} className={`flex-shrink-0 h-14 border-r border-slate-100 ${isWeekend ? "bg-brand-surface/60" : ""}`} style={{ width: CELL_W }} />;
                  })}
                  {roomBars.map((b) => {
      ...
      <div className="px-5 py-3 border-t border-slate-100 flex flex-wrap items-center gap-4 text-xs text-slate-500">
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-brand-primary"></span>Confirmed / Arriving</span>
