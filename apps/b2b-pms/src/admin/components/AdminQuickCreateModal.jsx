    ...
    <div className="fixed inset-0 z-[110] flex items-end md:items-center justify-center p-0 md:p-8 bg-slate-900/60 backdrop-blur-sm" onClick={onClose} data-testid={`quick-create-${entity}`}>
      <div onClick={(e) => e.stopPropagation()} className="bg-white w-full max-w-md rounded-t-[24px] md:rounded-[24px] p-8 shadow-[0_40px_100px_rgba(15,23,42,0.35)] relative max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3">
          <span className="w-10 h-10 rounded-full bg-brand-accent/12 text-brand-accent grid place-items-center"><i className={`fa-solid fa-${cfg.icon} text-sm`}></i></span>
          <div>
            <p className="text-eyebrow text-brand-accent">Quick create</p>
        ...
        <div className="mt-6 space-y-3">
          {cfg.fields.map((f) => (
            <label key={f.k} className="block">
              ...
              {f.type === "textarea" ? (
                <textarea rows={3} value={values[f.k]} onChange={(e) => set(f.k, e.target.value)} placeholder={f.placeholder} className="mt-1 w-full bg-brand-surface border border-slate-200 rounded-[12px] px-3 py-2 text-sm outline-none focus:border-brand-primary" data-testid={`qc-${entity}-${f.k}`} />
              ) : f.type === "select" ? (
                <select value={values[f.k]} onChange={(e) => set(f.k, e.target.value)} className="mt-1 w-full bg-brand-surface border border-slate-200 rounded-full px-3 py-2 text-sm outline-none focus:border-brand-primary" data-testid={`qc-${entity}-${f.k}`}>
                  <option value="">Choose…</option>
                  {f.options.map((o) => <option key={o} value={o}>{o}</option>)}
                </select>
              ) : (
                <input type={f.type} value={values[f.k]} onChange={(e) => set(f.k, e.target.value)} placeholder={f.placeholder} className="mt-1 w-full bg-brand-surface border border-slate-200 rounded-full px-3 py-2 text-sm outline-none focus:border-brand-primary" data-testid={`qc-${entity}-${f.k}`} />
              )}
            </label>
          ))}
        </div>
        ...
        <div className="mt-6 flex items-center justify-end gap-3">
          ...
          <button onClick={submit} className="px-5 py-2.5 rounded-full bg-brand-primary hover:bg-brand-primary-hover text-white text-sm shadow-[0_10px_28px_rgba(79,70,229,0.28)]" data-testid={`qc-${entity}-submit`}>
