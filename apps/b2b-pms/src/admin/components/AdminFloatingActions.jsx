  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-24 right-6 z-[55] w-14 h-14 rounded-full bg-brand-primary text-white shadow-[0_12px_32px_rgba(79,70,229,0.35)] hover:-translate-y-0.5 transition-all press-scale grid place-items-center print:hidden"
      ...
      >
        ...
        {unread > 0 && (
          <span
            className="absolute -top-1 -right-1 min-w-[22px] h-[22px] px-1 rounded-full bg-rose-500 text-white text-[10px] font-mono grid place-items-center border-2 border-brand-surface animate-pulse"
        ...
        <div className="fixed inset-0 z-[100] flex items-end sm:items-stretch sm:justify-end bg-slate-900/40 backdrop-blur-sm print:hidden" onClick={() => setOpen(false)} data-testid="admin-fab-panel">
          <div onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-md bg-white h-[85vh] sm:h-full rounded-t-[24px] sm:rounded-none flex flex-col shadow-[0_-20px_50px_rgba(15,23,42,0.25)]">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
              <span className="w-10 h-10 rounded-full bg-brand-accent/12 text-brand-accent grid place-items-center"><i className="fa-solid fa-wand-magic-sparkles"></i></span>
            ...
            {tab === "actions" && (
              <div className="p-5 space-y-2 overflow-y-auto">
                ...
                {visibleShortcuts.map((s) => (
                  <button key={s.entity} onClick={() => setQuick(s.entity)} className="w-full text-left p-4 rounded-[14px] bg-brand-surface hover:bg-white hover:shadow-sm border border-transparent hover:border-slate-200 flex items-center gap-3 transition-all" data-testid={`fab-shortcut-${s.entity}`}>
                    <span className="w-9 h-9 rounded-full bg-brand-primary/15 text-brand-primary grid place-items-center"><i className={`fa-solid fa-${s.i} text-sm`}></i></span>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {visibleJumps.map((j) => (
                        <button key={j.to} onClick={() => { setOpen(false); nav(j.to); }} className="p-2.5 rounded-[10px] bg-slate-100 hover:bg-slate-200 text-xs text-slate-700 flex items-center gap-2" data-testid={`fab-jump-${j.to.split("/").pop()}`}>
                          <i className={`fa-solid fa-${j.i} text-[10px] text-brand-accent`}></i>{j.l}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2" data-testid="fab-thread-body">
                      {activeThread.messages.map((m) => (
                        <div key={m.id} className={`flex ${m.from === "me" ? "justify-end" : "justify-start"}`}>
                          <div className={`max-w-[75%] px-3 py-2 rounded-[14px] text-sm ${m.from === "me" ? "bg-brand-primary text-white rounded-br-[6px]" : "bg-brand-surface text-slate-800 rounded-bl-[6px]"}`}>
                            <p>{m.text}</p>
                    ...
                    <div className="px-4 py-3 border-t border-slate-100 flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 bg-brand-surface border border-slate-200 rounded-full px-4 py-2 text-sm outline-none focus:border-brand-primary"
                        data-testid="fab-thread-input"
                      />
                      <button onClick={sendThread} className="w-9 h-9 rounded-full bg-brand-primary hover:bg-brand-primary-hover text-white grid place-items-center" aria-label="Send" data-testid="fab-thread-send">
                        <i className="fa-solid fa-paper-plane text-xs"></i>
                      </button>
                    </div>
                    <button
                      onClick={() => { setOpen(false); nav(t("admin/messages")); }}
                      className="mx-4 mb-3 text-[11px] text-brand-primary hover:underline text-left"
            ...
            {tab === "help" && (
              <>
                <div className="flex-1 overflow-y-auto p-4 space-y-3" data-testid="fab-chat">
                  {chat.map((m) => (
                    <div key={m.id} className={`flex ${m.from === "me" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] px-4 py-2.5 rounded-[14px] text-sm ${m.from === "me" ? "bg-brand-primary text-white rounded-br-[6px]" : "bg-brand-surface text-slate-800 rounded-bl-[6px]"}`}>{m.text}</div>
                    </div>
                  ))}
                </div>
                <div className="px-4 py-3 border-t border-slate-100">
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); send(); }} className="flex items-center gap-2 mt-2">
                    <input value={text} onChange={(e) => setText(e.target.value)} placeholder="Ask a question…" className="flex-1 bg-brand-surface border border-slate-200 rounded-full px-4 py-2 text-sm outline-none focus:border-brand-primary" data-testid="fab-input" />
                    <button type="submit" className="w-9 h-9 rounded-full bg-brand-primary hover:bg-brand-primary-hover text-white grid place-items-center" aria-label="Send" data-testid="fab-send"><i className="fa-solid fa-paper-plane text-xs"></i></button>
                  </form>
                </div>
              </>
            )}
