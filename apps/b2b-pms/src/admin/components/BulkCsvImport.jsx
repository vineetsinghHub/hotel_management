    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={onClose} data-testid="bulk-import">
      <div onClick={(e) => e.stopPropagation()} className="bg-white w-full max-w-3xl rounded-[20px] p-8 shadow-[0_40px_100px_rgba(15,23,42,0.35)] max-h-[92vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-eyebrow text-brand-accent">Bulk import</p>
        ...
        {!rows ? (
          <label className="mt-6 block border-2 border-dashed border-slate-200 rounded-[16px] p-10 text-center cursor-pointer hover:border-brand-primary transition-colors" data-testid="bulk-dropzone">
            <i className="fa-solid fa-file-csv text-4xl text-brand-accent"></i>
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-xs text-slate-600"><i className="fa-solid fa-file-csv text-brand-accent"></i>{fileName} · <span className="font-mono">{rows.length - 1}</span> rows</div>
        ...
        <div className="mt-6 flex items-center justify-end gap-3">
          ...
          <button onClick={commit} disabled={!rows} className="px-5 py-2.5 rounded-full bg-brand-primary hover:bg-brand-primary-hover text-white text-sm shadow-[0_10px_28px_rgba(79,70,229,0.28)] disabled:opacity-50 disabled:cursor-not-allowed" data-testid="bulk-commit">Import</button>
        </div>
      </div>
    </div>
  );
