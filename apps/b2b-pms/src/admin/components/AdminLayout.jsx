...
import { getAdminUser, clearAdminUser, seedUsers } from "@aura/shared/admin/adminAuth";
import { hasAccess, roleLabel, roleColor, isReadOnly } from "@aura/shared/admin/roles";
import { arrivals, roomsInventory, guests, notificationsAdmin } from "@aura/shared/admin/adminMockData";
import { isProModule, useTier } from "@aura/shared/admin/tier";
import { ProBadge } from "@aura/b2b-pms/admin/components/TierGate";
import AdminOnboardingTour from "@aura/b2b-pms/admin/components/AdminOnboardingTour";
import AdminQuickCreateModal from "@aura/b2b-pms/admin/components/AdminQuickCreateModal";
import AdminFloatingActions from "@aura/b2b-pms/admin/components/AdminFloatingActions";
...
export const AdminLayout = ({ pageTitle, children }) => {
  ...
  return (
    <div className="min-h-screen bg-brand-surface flex" data-testid="admin-layout">
      {/* Sidebar */}
                ...
                {visible.map((it) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {proBadge && <ProBadge />}
                      {it.badge != null && <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded-full ${active ? "bg-white/20 text-white" : "bg-brand-accent/15 text-brand-accent"}`}>{it.badge}</span>}
                    </Link>
                  );
                })}
              </div>
            ...
            <button onClick={() => setProfileOpen((v) => !v)} className="w-full flex items-center gap-3 p-2 rounded-[12px] hover:bg-slate-50" data-testid="profile-toggle">
              ...
              <div className="text-left flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-[8px] tracking-[0.15em] uppercase font-medium px-1.5 py-0.5 rounded-[4px] ${isPro ? "bg-gradient-to-r from-brand-accent to-brand-accent-hover text-slate-900" : "bg-slate-200 text-slate-600"}`} data-testid="tier-pill">
                    {isPro ? "Pro" : "Basic"}
                  </span>
                </div>
              </div>
...
const TopbarAndMain = ({ pageTitle, onOpenMobile, children }) => {
  ...
  return (
    <div className="flex-1 lg:ml-64">
      <header className="sticky top-0 z-30 bg-brand-surface/80 backdrop-blur-xl border-b border-slate-200 px-6 py-4 flex items-center justify-between">
            ...
            {notifOpen && (
              <div className="absolute right-0 top-full mt-2 w-96 bg-white border border-slate-200 rounded-[16px] shadow-[0_20px_50px_rgba(15,23,42,0.10)] p-4 z-40" data-testid="notif-dropdown">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => { setNotifs((s) => s.map((n) => ({ ...n, read: true }))); toast.success("All read"); }} className="text-xs text-brand-primary hover:underline" data-testid="topbar-mark-all">Mark all read</button>
                </div>
                <ul className="space-y-2 max-h-80 overflow-y-auto">
                  {notifs.slice(0, 5).map((n) => (
                    <li key={n.id} className={`p-3 rounded-[12px] flex items-start gap-3 ${!n.read ? "bg-indigo-50/40" : ""}`}>
                      <span className="w-8 h-8 rounded-full bg-brand-primary/12 text-brand-primary grid place-items-center"><i className="fa-solid fa-bell text-[11px]"></i></span>
                ...
                <Link to="/admin/notifications" onClick={() => setNotifOpen(false)} className="mt-3 block text-center text-xs text-brand-primary hover:underline">Open notifications center →</Link>
              </div>
            )}
          </div>
          <div className="relative">
            {newItems.length > 0 && (
              <button onClick={() => setNewOpen((v) => !v)} className="inline-flex items-center gap-2 bg-brand-primary hover:bg-brand-primary-hover text-white text-sm px-4 py-2 rounded-full shadow-[0_6px_20px_rgba(79,70,229,0.28)]" data-testid="new-btn"><i className="fa-solid fa-plus text-[10px]"></i>New</button>
            )}
            {newOpen && (
              <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-slate-200 rounded-[16px] shadow-[0_20px_50px_rgba(15,23,42,0.10)] py-2 z-40" data-testid="new-menu">
                ...
                {newItems.map((it) => (
                  <button key={it.l} onClick={() => { setNewOpen(false); setQuickEntity(it.entity); }} className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-3" data-testid={`new-${it.entity}`}>
                    <span className="w-7 h-7 rounded-full bg-brand-accent/12 text-brand-accent grid place-items-center"><i className={`fa-solid fa-${it.i} text-[11px]`}></i></span>
                    {it.l}
                  </button>
                ))}
              </div>
        ...
        <div className="max-h-96 overflow-y-auto">
          {!q.trim() ? (
            ...
            <ul>
              {results.map((r, i) => (
                <li key={i}>
                  <button onClick={() => { nav(r.to); onClose(); }} className="w-full text-left px-5 py-3 hover:bg-brand-surface flex items-center gap-3 border-t border-slate-100" data-testid={`search-result-${i}`}>
  ...
  <div className="fixed inset-0 z-[100] flex items-stretch justify-end bg-slate-900/60 backdrop-blur-sm" onClick={onClose} data-testid="help-drawer">
    <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-white h-full overflow-y-auto p-8 shadow-[0_40px_100px_rgba(15,23,42,0.35)]">
      <div className="flex items-center justify-between">
        <div><p className="text-eyebrow text-brand-accent">Help</p><h3 className="mt-1 font-serif text-2xl text-slate-900">Shortcuts & Docs</h3></div>
      ...
      <div className="mt-8 p-4 rounded-[14px] bg-brand-surface border border-slate-100">
