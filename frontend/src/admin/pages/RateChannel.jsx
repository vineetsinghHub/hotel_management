...
import { otaChannels, rateCategories, statusColor } from "@/admin/adminMockData";
  ...
  return (
    <AdminLayout pageTitle="Rate & Channel Manager">
      ...
      <div className="border-b border-slate-200 flex items-center gap-2 mb-6 overflow-x-auto">
        <button onClick={() => setTab("connections")} className={`px-4 py-3 text-sm border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${tab === "connections" ? "border-[#4F46E5] text-slate-900 font-medium" : "border-transparent text-slate-500 hover:text-slate-800"}`} data-testid="rc-tab-connections">
          <i className="fa-solid fa-link text-[11px]"></i>OTA Connections
          <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-full ${tab === "connections" ? "bg-[#C9A227]/15 text-[#C9A227]" : "bg-slate-100 text-slate-500"}`}>{connCount}</span>
          {errorCount > 0 && <span className="text-[10px] font-mono px-1.5 py-0.5 rounded-full bg-rose-100 text-rose-700">{errorCount}</span>}
        </button>
        <button onClick={() => setTab("rates")} className={`px-4 py-3 text-sm border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${tab === "rates" ? "border-[#4F46E5] text-slate-900 font-medium" : "border-transparent text-slate-500 hover:text-slate-800"}`} data-testid="rc-tab-rates">
      ...
      {bulkOpen && selection && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" data-testid="bulk-rate-modal">
          <div className="bg-white w-full max-w-md rounded-[20px] p-8 relative">
            ...
            <p className="text-eyebrow text-[#C9A227]">Bulk edit</p>
            ...
            <div className="mt-5">
              ...
              <input autoFocus value={bulkValue} onChange={(e) => setBulkValue(e.target.value)} placeholder="e.g. 32000" className="mt-2 w-full bg-[#FAFAF8] border border-slate-200 rounded-[14px] px-4 py-3 text-sm outline-none focus:border-[#4F46E5] font-mono" data-testid="bulk-rate-input" />
            </div>
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button onClick={applyBulk} className="px-5 py-2.5 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white text-sm" data-testid="bulk-rate-apply">Push to all channels</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div key={c.id} className="bg-white rounded-[18px] border border-slate-200 p-6 flex flex-col" data-testid={`channel-card-${c.id}`}>
            ...
            <div className="mt-4 flex items-center gap-2">
              <button onClick={() => onOpen(c)} className="flex-1 text-xs px-3 py-2 rounded-full border border-slate-200 hover:bg-slate-50" data-testid={`channel-manage-${c.id}`}>
                <i className="fa-solid fa-sliders mr-1.5 text-[10px] text-[#C9A227]"></i>Manage mapping
              </button>
              {c.status === "sync_error" && (
              ...
              {c.status !== "sync_error" && (
                <button onClick={() => onToggle(c.id)} className={`text-xs px-3 py-2 rounded-full ${c.status === "connected" ? "border border-slate-200 hover:bg-slate-50" : "bg-[#4F46E5] text-white hover:bg-[#4338CA]"}`} data-testid={`channel-toggle-${c.id}`}>
  ...
  return (
    <div className="bg-white rounded-[16px] border border-slate-200 overflow-hidden" data-testid="rate-calendar">
      <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <p className="text-eyebrow text-[#C9A227]">Master Rate Calendar</p>
        ...
        <div className="flex items-center gap-2 text-xs flex-wrap">
          ...
          {selection && (
            <button onClick={onBulk} className="px-3 py-1.5 rounded-full bg-[#C9A227] hover:bg-[#B08D1E] text-slate-900 font-medium" data-testid="rate-bulk-btn">
              <i className="fa-solid fa-pen-to-square mr-1 text-[10px]"></i>Edit {Math.abs(selection.to - selection.from) + 1} nights
            </button>
          )}
          <button onClick={onPush} className="px-3 py-1.5 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white" data-testid="rate-push-all">
              ...
              {dates.map((d, i) => {
                ...
                return (
                  <div
                    ...
                    className={`flex-shrink-0 h-14 border-r border-slate-100 grid place-items-center cursor-pointer transition-all select-none ${selected ? "bg-[#4F46E5]/10 ring-1 ring-inset ring-[#4F46E5]/40" : isWeekend ? "bg-[#FAFAF8]/60 hover:bg-[#FAFAF8]" : "hover:bg-slate-50"}`}
                    style={{ width: CELL_W }}
                  ...
                  >
                    {isEditing ? (
                      <input
                        ...
                        className="w-full h-full text-center font-mono text-xs bg-white border border-[#4F46E5] rounded-[6px] outline-none"
                        data-testid="rate-edit-input"
                      />
                    ) : (
                      <span className={`font-mono text-xs ${value > cat.baseRate * 1.2 ? "text-[#C9A227]" : "text-slate-800"}`}>₹{(value / 1000).toFixed(1)}K</span>
                    )}
                  </div>
                );
              })}
      ...
      <div className="px-5 py-3 border-t border-slate-100 flex flex-wrap items-center gap-4 text-xs text-slate-500">
        ...
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#4F46E5]/20 ring-1 ring-[#4F46E5]/40"></span>Selected</span>
        <span className="flex items-center gap-1.5"><span className="font-mono text-[#C9A227]">₹XX.XK</span>Surge rate</span>
    ...
    <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-white h-full overflow-y-auto p-8 shadow-[0_40px_100px_rgba(15,23,42,0.35)]">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-eyebrow text-[#C9A227]">Channel mapping</p>
            ...
            <div key={cat.id} className="p-4 rounded-[14px] border border-slate-200" data-testid={`mapping-${cat.id}`}>
              <div className="flex items-center justify-between">
                ...
                <input
                  ...
                  className={`bg-[#FAFAF8] border border-slate-200 rounded-[10px] px-3 py-2 text-xs outline-none focus:border-[#4F46E5] font-mono w-40 ${i >= channel.mapped ? "border-dashed" : ""}`}
                />
              </div>
            </div>
          ))}
      ...
      <div className="mt-6 p-4 rounded-[14px] bg-[#FAFAF8] border border-slate-100">
        <p className="text-eyebrow text-slate-500">Rate parity</p>
        <p className="mt-2 text-sm text-slate-800">Publish Aura Direct rate + <span className="font-mono text-[#4F46E5]">{channel.commission}%</span> for parity, or use channel-specific pricing.</p>
      ...
      <div className="mt-6 flex items-center gap-2 justify-end">
        ...
        <button onClick={() => { toast.success(`${channel.name} mapping saved`); onClose(); }} className="text-xs px-5 py-2 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white" data-testid="channel-drawer-save">Save & sync</button>
      </div>
    </div>
  </div>
);
