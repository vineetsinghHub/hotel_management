  ...
  return (
    <AdminLayout pageTitle="Events & Weddings">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-eyebrow text-[#C9A227]">{events.length} contracted events</p>
        ...
        {!readOnly && (
          <button
            ...
            className="inline-flex items-center gap-2 bg-[#4F46E5] hover:bg-[#4338CA] text-white text-sm px-5 py-2.5 rounded-full shadow-[0_6px_20px_rgba(79,70,229,0.28)]"
        ...
        {events.map((e) => (
          <div key={e.id} className="p-6 bg-white rounded-[16px] border border-slate-200" data-testid={`event-${e.id}`}>
            <div className="flex items-start justify-between">
              <div>
                <p className="text-eyebrow text-[#C9A227]">{e.type}</p>
