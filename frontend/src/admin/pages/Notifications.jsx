...
import { notificationsAdmin } from "@/admin/adminMockData";
  ...
  return (
    <AdminLayout pageTitle="Notifications">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={markAll} className="text-xs text-[#4F46E5] hover:underline" data-testid="mark-all">Mark all as read</button>
      </div>
      <div className="bg-white rounded-[16px] border border-slate-200 divide-y divide-slate-100">
        {rows.map((n) => (
          <div key={n.id} className={`p-5 flex items-start gap-4 ${!n.read ? "bg-indigo-50/30" : ""}`} data-testid={`notif-${n.id}`}>
            <span className="w-10 h-10 rounded-full bg-[#4F46E5]/10 text-[#4F46E5] grid place-items-center flex-shrink-0"><i className={`fa-solid fa-${iconOf(n.kind)} text-sm`}></i></span>
