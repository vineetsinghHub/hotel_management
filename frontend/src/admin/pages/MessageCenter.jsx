...
import { useTier } from "@/admin/tier";
  ...
  return (
    <AdminLayout pageTitle="Messages">
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <div>
          <p className="text-eyebrow text-[#C9A227]">Communications</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr_280px] gap-3 h-[calc(100vh-240px)] min-h-[520px]" data-testid="message-center">
        ...
        <aside className="bg-white rounded-[16px] border border-slate-200 overflow-hidden flex flex-col">
          <div className="p-3 border-b border-slate-100">
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={`Search ${segment === "guests" ? "guest" : "team"} threads…`} className="w-full bg-[#FAFAF8] border border-transparent rounded-full px-4 py-2 text-sm outline-none focus:border-slate-200" data-testid="msg-search" />
          </div>
        ...
        <section className="bg-white rounded-[16px] border border-slate-200 flex flex-col" data-testid="msg-thread">
          {segment === "guests" && activeGuest ? (
              ...
              <header className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  {activeChannels.map((c) => (
                    <button key={c.k} onClick={() => setChannel(c.k)} className={`w-8 h-8 rounded-full grid place-items-center ${channel === c.k ? "bg-[#4F46E5] text-white" : "text-slate-500 hover:bg-slate-50"}`} data-testid={`channel-${c.k}`} aria-label={c.k}>
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3" data-testid="team-thread-body">
                {activeTeam.messages.map((m) => (
                  <div key={m.id} className={`flex ${m.from === "me" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[75%] px-4 py-2.5 rounded-[16px] text-sm ${m.from === "me" ? "bg-slate-900 text-white rounded-br-[6px]" : "bg-[#FAFAF8] text-slate-800 rounded-bl-[6px]"}`}>
                      <p>{m.text}</p>
              ...
              <div className="px-4 py-3 border-t border-slate-100 flex items-center gap-2">
                <input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder={`Message ${activeTeam.name.split(" ")[0]}…`} className="flex-1 bg-[#FAFAF8] border border-slate-200 rounded-full px-4 py-2.5 text-sm outline-none focus:border-slate-900" data-testid="team-msg-input" />
        ...
        <aside className="bg-white rounded-[16px] border border-slate-200 p-4 overflow-y-auto">
          ...
          {segment === "team" && activeTeam && (
            <>
              <p className="text-eyebrow text-[#C9A227]">Colleague</p>
              <div className="mt-2 flex items-center gap-3">
