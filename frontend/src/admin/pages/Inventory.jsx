              ...
              return (
                <tr key={i.id} className="border-t border-slate-100 hover:bg-slate-50" data-testid={`inv-${i.id}`}>
                  <td className="px-5 py-2">
                    {isEditing ? (
                      <input value={i.item} onChange={(e) => updateField(i.id, "item", e.target.value)} className="bg-white border border-slate-200 rounded-full px-3 py-1 text-sm outline-none focus:border-[#4F46E5] w-48" data-testid={`inv-edit-item-${i.id}`} />
                    ) : (
      ...
      {addOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={() => setAddOpen(false)} data-testid="inv-add-modal">
          <div onClick={(e) => e.stopPropagation()} className="bg-white w-full max-w-md rounded-[20px] p-6 shadow-[0_40px_100px_rgba(15,23,42,0.35)]">
            <p className="text-eyebrow text-[#C9A227]">Add stock</p>
            <h3 className="mt-1 font-serif text-xl text-slate-900">New inventory item</h3>
            <div className="mt-4 space-y-3">
              <input value={form.item} onChange={(e) => setForm({ ...form, item: e.target.value })} placeholder="Item name" className="w-full bg-[#FAFAF8] border border-slate-200 rounded-full px-3 py-2 text-sm outline-none focus:border-[#4F46E5]" data-testid="inv-form-item" />
              <div className="grid grid-cols-2 gap-3">
                <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full bg-[#FAFAF8] border border-slate-200 rounded-full px-3 py-2 text-sm">{["F&B", "Linen", "Amenities", "Cleaning", "Office", "Spa"].map((c) => <option key={c} value={c}>{c}</option>)}</select>
                <input value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} placeholder="Unit" className="w-full bg-[#FAFAF8] border border-slate-200 rounded-full px-3 py-2 text-sm" />
              </div>
              <div className="grid grid-cols-3 gap-3">
                <input type="number" value={form.stock} onChange={(e) => setForm({ ...form, stock: e.target.value })} placeholder="Stock" className="w-full bg-[#FAFAF8] border border-slate-200 rounded-full px-3 py-2 text-sm" />
                <input type="number" value={form.par} onChange={(e) => setForm({ ...form, par: e.target.value })} placeholder="Par" className="w-full bg-[#FAFAF8] border border-slate-200 rounded-full px-3 py-2 text-sm" />
                <input type="number" value={form.cost} onChange={(e) => setForm({ ...form, cost: e.target.value })} placeholder="Cost ₹" className="w-full bg-[#FAFAF8] border border-slate-200 rounded-full px-3 py-2 text-sm" />
              </div>
            </div>
            <div className="mt-5 flex items-center justify-end gap-3">
              <button onClick={() => setAddOpen(false)} className="px-4 py-2 rounded-full border border-slate-200 text-sm">Cancel</button>
              <button onClick={addItem} className="px-4 py-2 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white text-sm" data-testid="inv-form-submit">Add item</button>
            </div>
          </div>
        </div>
      )}
