...
const AuditPanel = () => {
  ...
  return (
    <div className="p-6 bg-white rounded-[16px] border border-slate-200" data-testid="audit-panel">
      ...
      {/* Search */}
      <div className="mt-4 flex items-center gap-2 bg-[#FAFAF8] border border-slate-200 rounded-full px-3 py-2">
      ...
      <div className="mt-2 grid grid-cols-2 gap-2">
        <select value={roleF} onChange={(e) => setRoleF(e.target.value)} className="bg-[#FAFAF8] border border-slate-200 rounded-full px-3 py-1.5 text-[11px] outline-none focus:border-[#4F46E5]" data-testid="audit-role-filter">
          <option value="all">All actors</option>
          {ROLES.map((r) => <option key={r.key} value={r.key}>{r.label}</option>)}
        </select>
        <select value={actionF} onChange={(e) => setActionF(e.target.value)} className="bg-[#FAFAF8] border border-slate-200 rounded-full px-3 py-1.5 text-[11px] outline-none focus:border-[#4F46E5]" data-testid="audit-action-filter">
        ...
        {filtered.length === 0 ? (
          ...
          <li key={e.id} className="pb-3 border-b border-slate-100 last:border-0" data-testid={`audit-row-${e.id}`}>
            ...
            <p className="mt-1 text-slate-600">
              <span className="text-[9px] tracking-widest uppercase text-[#C9A227] mr-1.5">{AUDIT_ACTION_LABELS[e.action] || e.action}</span>
              {e.target}
            </p>
