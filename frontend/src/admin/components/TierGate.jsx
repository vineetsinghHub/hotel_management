    ...
    <div className={inline ? "" : "min-h-[calc(100vh-160px)] grid place-items-center px-4 py-10"} data-testid={`tier-gate-${routeKey}`}>
      <div className={`relative w-full ${compact ? "max-w-2xl" : "max-w-3xl"}`}>
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-[24px] border border-slate-200 overflow-hidden shadow-[0_40px_100px_rgba(15,23,42,0.12)]">
          {/* Hero band */}
          <div className={`relative ${compact ? "px-6 pt-6 pb-5" : "px-8 pt-10 pb-8"} bg-gradient-to-br from-slate-900 via-[#312E81] to-[#4F46E5] text-white overflow-hidden`}>
            ...
            <div className={`relative flex items-start ${compact ? "gap-4" : "gap-5"}`}>
              <span className={`${compact ? "w-12 h-12 rounded-[12px]" : "w-16 h-16 rounded-[16px]"} bg-white/10 backdrop-blur border border-white/15 grid place-items-center flex-shrink-0`}>
                <i className={`fa-solid fa-${info.icon} ${compact ? "text-xl" : "text-2xl"} text-[#E6C868]`}></i>
              </span>
              <div>
          ...
          <div className={compact ? "px-6 py-5" : "px-8 py-8"}>
            <div className={`${compact ? "p-3" : "p-4"} rounded-[14px] bg-[#4F46E5]/8 border border-[#4F46E5]/15 flex gap-3 items-start`}>
            ...
            <div className={`${compact ? "mt-4" : "mt-6"} grid grid-cols-1 md:grid-cols-2 gap-4`}>
              {/* Current */}
              <div className={`${compact ? "p-4" : "p-5"} rounded-[16px] border border-slate-200 bg-[#FAFAF8]`}>
              ...
              {/* Pro */}
              <div className={`${compact ? "p-4" : "p-5"} rounded-[16px] border-2 border-[#4F46E5] bg-gradient-to-br from-white to-[#F5F3FF] relative`}>
            ...
            <div className={`${compact ? "mt-5" : "mt-8"} flex flex-col sm:flex-row items-center justify-between gap-3`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={upgrade}
                  disabled={busy}
                  className={`${compact ? "px-5 py-2" : "px-6 py-2.5"} rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white text-sm font-medium shadow-[0_10px_28px_rgba(79,70,229,0.35)] flex items-center gap-2 disabled:opacity-70`}
