      ...
      <div className="p-4 rounded-[16px] bg-white border border-slate-200">
        <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {presets.map((p) => (
              <button key={p.k} onClick={() => applyPreset(p)} className="px-3 py-1.5 rounded-full bg-[#FAFAF8] hover:bg-slate-100 text-xs text-slate-700" data-testid={`preset-${p.k}`}>{p.name}</button>
            ))}
          </div>
          <div className="flex items-center gap-2">
            <label className="text-eyebrow text-slate-500">Range</label>
            <select value={range} onChange={(e) => setRange(Number(e.target.value))} className="px-3 py-1.5 rounded-full bg-[#FAFAF8] text-xs border-none outline-none" data-testid="report-range">
              <option value={7}>7 days</option><option value={30}>30 days</option><option value={90}>90 days</option>
            </select>
            <button onClick={exportCsv} className="px-4 py-2 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white text-xs shadow-[0_10px_28px_rgba(79,70,229,0.28)]" data-testid="report-export"><i className="fa-regular fa-arrow-down-to-line text-[10px] mr-1.5"></i>Export CSV</button>
          </div>
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          {allFields.map((f) => (
            <button key={f.k} onClick={() => toggle(f.k)} className={`px-3 py-1.5 rounded-full text-xs border transition-all ${fields.includes(f.k) ? "bg-[#4F46E5] text-white border-[#4F46E5]" : "bg-white text-slate-500 border-slate-200 hover:border-slate-300"}`} data-testid={`field-${f.k}`}>
