        ...
        <div className="p-6 md:p-8">
          <div className="flex flex-col md:flex-row md:items-center gap-5">
            ...
            <div className="flex-1">
              <p className="text-eyebrow text-[#C9A227]">{guest.tier || "Aura Circle"}</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              <div className="p-3 rounded-[12px] bg-[#FAFAF8] text-center">
              ...
              <div className="p-3 rounded-[12px] bg-[#FAFAF8] text-center col-span-2 md:col-span-1">
          ...
          <div className="mt-8 border-b border-slate-100 flex gap-1 overflow-x-auto" role="tablist">
            {tabs.map((tb) => (
              <button key={tb.k} onClick={() => setTab(tb.k)} className={`px-4 py-2.5 text-sm border-b-2 transition-colors flex items-center gap-2 ${tab === tb.k ? "border-[#4F46E5] text-slate-900" : "border-transparent text-slate-500 hover:text-slate-900"}`} data-testid={`g360-tab-${tb.k}`} role="tab" aria-selected={tab === tb.k}>
          ...
          <div className="mt-6">
            {tab === "overview" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 rounded-[16px] bg-[#FAFAF8]"><p className="text-eyebrow text-slate-500">Home</p><p className="mt-1 text-slate-900">{guest.country || "Mumbai, India"}</p></div>
                <div className="p-4 rounded-[16px] bg-[#FAFAF8]"><p className="text-eyebrow text-slate-500">Last stay</p><p className="mt-1 text-slate-900">{mockStays[0].dates}</p></div>
                <div className="p-4 rounded-[16px] bg-[#FAFAF8]"><p className="text-eyebrow text-slate-500">Lifetime value</p><p className="mt-1 text-slate-900 font-mono">${totalSpend.toLocaleString()}</p></div>
                <div className="p-4 rounded-[16px] bg-[#FAFAF8]"><p className="text-eyebrow text-slate-500">Preferred channel</p><p className="mt-1 text-slate-900">WhatsApp</p></div>
              </div>
            )}
            {tab === "stays" && (
              <ul className="divide-y divide-slate-100 border border-slate-100 rounded-[16px] overflow-hidden" data-testid="g360-stays">
                {mockStays.map((s) => (
                  <li key={s.id} className="p-4 flex flex-wrap items-center gap-4">
                    ...
                    <div className="flex">{[1,2,3,4,5].map((n) => <i key={n} className={`fa-${n <= s.rating ? "solid" : "regular"} fa-star text-xs text-[#C9A227]`}></i>)}</div>
                  </li>
                ))}
              </ul>
            )}
            {tab === "prefs" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3" data-testid="g360-prefs">
                {mockPrefs.map((p) => (
                  <div key={p.key} className="p-4 rounded-[16px] bg-[#FAFAF8] flex items-start justify-between">
                    <div><p className="text-eyebrow text-slate-500">{p.label}</p><p className="mt-1 text-slate-900 text-sm">{p.value}</p></div>
                    <button className="text-xs text-[#4F46E5] hover:underline">Edit</button>
                  </div>
                ))}
              </div>
            )}
              ...
              <ul className="space-y-3" data-testid="g360-messages">
                {mockThread.map((m) => (
                  <li key={m.id} className={`flex ${m.from === "guest" ? "justify-start" : "justify-end"}`}>
                    <div className={`max-w-md px-4 py-2.5 rounded-[16px] text-sm ${m.from === "guest" ? "bg-[#FAFAF8] text-slate-800 rounded-bl-[6px]" : "bg-[#4F46E5] text-white rounded-br-[6px]"}`}>
                      <p>{m.text}</p>
            ...
            {tab === "loyalty" && (
              <div className="p-5 rounded-[18px] bg-gradient-to-br from-[#4F46E5] to-slate-900 text-white" data-testid="g360-loyalty">
                ...
                <div className="mt-3 w-full h-2 rounded-full bg-white/10 overflow-hidden"><div className="h-full w-2/3 bg-[#E6C868]"></div></div>
