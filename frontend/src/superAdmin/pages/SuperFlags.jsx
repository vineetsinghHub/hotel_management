...
import { featureFlags as seed, platformTenants } from "@/superAdmin/superAdminMockData";
          ...
          <div key={f.key} className="p-5 rounded-[16px] bg-white/5 border border-white/10" data-testid={`flag-${f.key}`}>
            ...
            {expanded === f.key && (
              <div className="mt-4 pt-4 border-t border-white/5" data-testid={`flag-panel-${f.key}`}>
                <p className="text-eyebrow text-[#C9A227] mb-3">Tenant overrides</p>
                  ...
                  {platformTenants.slice(0, 8).map((t) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs border transition-colors ${on ? "border-[#4F46E5] bg-[#4F46E5]/15 text-white" : "border-white/10 bg-white/5 text-slate-400 hover:bg-white/10"}`}
