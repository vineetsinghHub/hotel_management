...
export default function SuperTenants() {
  ...
  const columns = useMemo(() => [
        ...
        return (
          <Link to={`/super-admin/tenants/${t.slug}`} className="flex items-center gap-3 group" data-testid={`tenant-row-${t.slug}`}>
            ...
            <div className="min-w-0">
              <p className="text-white text-sm truncate group-hover:text-[#E6C868]">{t.brandName}</p>
    ...
    columnHelper.accessor("tier", {
      header: "Tier",
      cell: (i) => (
        <span className={`text-[10px] tracking-widest uppercase px-2 py-0.5 rounded font-medium ${i.getValue() === "pro" ? "bg-gradient-to-r from-[#C9A227] to-[#E6C868] text-slate-900" : "bg-slate-700 text-slate-300"}`}>
          {i.getValue()}
        </span>
      ),
    }),
  ...
  const filterPill = (value, current, onClick, label) => (
    <button
      onClick={onClick}
      className={`px-3 py-1.5 rounded-full text-xs capitalize transition-colors ${current === value ? "bg-[#4F46E5] text-white" : "bg-white/5 text-slate-300 hover:bg-white/10"}`}
  ...
  return (
    <SuperAdminLayout
      pageTitle="Tenants"
      rightSlot={
        <Link to="/super-admin/provision" className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white text-sm shadow-[0_10px_28px_rgba(79,70,229,0.35)]" data-testid="new-tenant-btn">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5" data-testid="tenants-filters">
        <div className="relative flex-1 min-w-[220px] max-w-md">
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-full pl-10 pr-4 py-2.5 text-sm text-white outline-none focus:border-[#4F46E5]"
