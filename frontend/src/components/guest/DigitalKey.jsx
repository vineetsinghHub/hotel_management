...
export const DigitalKey = ({ suite = "Maharajah Suite", roomNumber = "204", resCode = "AH-9F27C1" }) => {
  ...
  return (
    <section className="relative overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-[#0F172A] via-[#1E1B4B] to-[#312E81] text-white p-8 md:p-10" data-testid="digital-key">
      ...
      <div aria-hidden="true" className="absolute -right-24 -top-24 w-72 h-72 rounded-full border border-[#C9A227]/30"></div>
      <div aria-hidden="true" className="absolute -right-40 -top-40 w-96 h-96 rounded-full border border-[#C9A227]/15"></div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        <div>
          <p className="text-eyebrow text-[#E6C868]">Digital Key</p>
          ...
          <div className="mt-6 flex items-center gap-3">
            <div className="relative w-14 h-14 grid place-items-center">
              <span className="absolute inset-0 rounded-full border-2 border-[#C9A227]/60 animate-ping"></span>
              <span className="absolute inset-2 rounded-full border-2 border-[#C9A227]/40 animate-ping" style={{ animationDelay: "0.4s" }}></span>
              <span className="relative w-8 h-8 rounded-full bg-[#C9A227] text-slate-900 grid place-items-center"><i className="fa-solid fa-wave-square text-[11px]"></i></span>
            </div>
            <div>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <button onClick={unlock} className="px-5 py-2.5 rounded-full bg-[#C9A227] hover:bg-[#B08D1E] text-slate-900 text-sm font-medium" data-testid="digital-key-unlock"><i className="fa-solid fa-unlock text-[10px] mr-1.5"></i>Unlock room</button>
