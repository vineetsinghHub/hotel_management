...
const SlotColumn = ({ slot, items, onRemove }) => {
  ...
  return (
    <div ref={setNodeRef} className={`p-3 rounded-[16px] border border-dashed transition-colors ${isOver ? "border-[#4F46E5] bg-indigo-50/50" : "border-slate-200 bg-[#FAFAF8]"}`} data-testid={`itin-slot-${slot.toLowerCase()}`}>
      <p className="text-eyebrow text-[#C9A227] mb-2">{slot}</p>
  ...
  return (
    <section className="bg-white rounded-[28px] border border-slate-200 p-8 md:p-10" data-testid="itinerary-planner">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <p className="text-eyebrow text-[#C9A227]">Curate your day</p>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => toast.success("Itinerary saved & sent to concierge")} className="px-4 py-2 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white text-xs shadow-[0_10px_28px_rgba(79,70,229,0.28)]" data-testid="itin-save">Save</button>
        </div>
      </div>
      ...
      <DndContext sensors={sensors} onDragStart={(e) => setDragItem(e.active.data.current)} onDragEnd={onDragEnd} onDragCancel={() => setDragItem(null)}>
        ...
        <DragOverlay>
          {dragItem ? (
            <div className="p-3 rounded-[14px] border border-[#4F46E5] bg-white shadow-[0_20px_50px_rgba(79,70,229,0.35)]">
              <div className="flex items-center gap-3">
