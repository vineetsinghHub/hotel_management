  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-36 right-6 z-[55] w-14 h-14 rounded-full bg-[#4F46E5] text-white shadow-[0_12px_32px_rgba(79,70,229,0.35)] hover:-translate-y-0.5 transition-all press-scale grid place-items-center print:hidden"
        aria-label="Open concierge chat"
        data-testid="concierge-open"
      >
        <i className="fa-solid fa-comments text-lg" aria-hidden="true"></i>
        <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[#C9A227] border-2 border-white text-[9px] text-slate-900 font-mono grid place-items-center">1</span>
      </button>
          ...
          <div onClick={(e) => e.stopPropagation()} className="w-full sm:max-w-md bg-white h-[85vh] sm:h-full rounded-t-[24px] sm:rounded-none flex flex-col shadow-[0_-20px_50px_rgba(15,23,42,0.25)]">
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3" data-testid="concierge-messages">
              {messages.map((m) => (
                <div key={m.id} className={`flex ${m.from === "me" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[80%] px-4 py-2.5 rounded-[16px] text-sm leading-relaxed ${m.from === "me" ? "bg-[#4F46E5] text-white rounded-br-[6px]" : "bg-[#FAFAF8] text-slate-800 rounded-bl-[6px]"}`}>
                    {m.text}
                  </div>
                </div>
              ))}
              {typing && (
                <div className="flex justify-start">
                  <div className="px-4 py-3 rounded-[16px] bg-[#FAFAF8] text-slate-500 text-sm flex gap-1">
            ...
            <div className="px-4 py-3 border-t border-slate-100">
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {quickActions.map((a) => (
                  <button key={a.label} onClick={() => send(a.label)} className="px-2.5 py-1.5 rounded-full bg-[#FAFAF8] border border-slate-100 hover:bg-slate-100 text-[11px] text-slate-700 flex items-center gap-1.5" data-testid={`quick-${a.icon}`}>
                    <i className={`fa-solid fa-${a.icon} text-[10px] text-[#C9A227]`} aria-hidden="true"></i>{a.label}
                  </button>
                ))}
              </div>
              <form onSubmit={(e) => { e.preventDefault(); send(text); }} className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 bg-[#FAFAF8] border border-slate-200 rounded-full px-4 py-2.5 text-sm outline-none focus:border-[#4F46E5]"
                  data-testid="concierge-input"
                  aria-label="Message concierge"
                />
                <button type="submit" className="w-10 h-10 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white grid place-items-center" aria-label="Send" data-testid="concierge-send">
