  ...
  return (
    <section className="bg-white rounded-[28px] border border-slate-200 p-8 md:p-10" data-testid="room-service-section">
      <div className="flex flex-col md:flex-row md:items-center gap-4 justify-between">
        <div>
          <p className="text-eyebrow text-[#C9A227]">In-Suite Dining</p>
        ...
        <div className="inline-flex rounded-full bg-[#FAFAF8] p-1 gap-1" role="tablist">
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-3">
          {items.map((it) => (
            <div key={it.id} className="p-4 rounded-[16px] bg-[#FAFAF8] hover:bg-white border border-transparent hover:border-slate-200 flex items-center gap-4 transition-all" data-testid={`menu-item-${it.id}`}>
              <span className="w-11 h-11 rounded-full bg-[#C9A227]/12 text-[#C9A227] grid place-items-center"><i className={`fa-solid fa-${it.icon} text-sm`}></i></span>
              ...
              <div className="flex items-center gap-2">
                ...
                {cart[it.id] ? (
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => inc(it.id)} className="w-7 h-7 rounded-full bg-[#4F46E5] text-white grid place-items-center" aria-label={`Add one ${it.name}`} data-testid={`menu-inc-${it.id}`}><i className="fa-solid fa-plus text-[9px]"></i></button>
                  </div>
                ) : (
        ...
        <aside className="p-5 rounded-[16px] bg-[#0F172A] text-white" data-testid="room-service-cart">
          <p className="text-eyebrow text-[#E6C868]">Your Cart</p>
          {cartItems.length === 0 ? (
            ...
            <>
              ...
              <div className="mt-4">
                ...
                <div className="mt-2 grid grid-cols-4 gap-1">
                  {[0, 5, 10, 15].map((p) => (
                    <button key={p} onClick={() => setTipPct(p)} className={`py-1.5 rounded-full text-xs ${tipPct === p ? "bg-[#C9A227] text-slate-900" : "bg-white/5 text-white/80"}`} data-testid={`tip-${p}`}>{p}%</button>
                  ))}
                </div>
              </div>
              <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2} placeholder="Notes (allergies, timing…)" className="mt-4 w-full bg-white/5 border border-white/10 rounded-[12px] px-3 py-2 text-sm outline-none focus:border-[#C9A227]/60" data-testid="room-service-note" />
              ...
              <button onClick={send} className="mt-4 w-full py-3 rounded-full bg-[#4F46E5] hover:bg-[#4338CA] text-white text-sm shadow-[0_10px_28px_rgba(79,70,229,0.28)]" data-testid="room-service-send">
