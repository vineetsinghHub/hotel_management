        ...
        <div className="max-h-96 overflow-y-auto">
          {!q.trim() ? (
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 gap-2">
                {pages.slice(0, 6).map((p) => (
                  <button key={p.to} onClick={() => { nav(p.to); setOpen(false); }} className="text-left px-3 py-2 rounded-[10px] hover:bg-[#FAFAF8] text-slate-700 text-sm" data-testid={`guest-palette-quick-${p.to.slice(1) || "home"}`}>{p.l}</button>
                ))}
              </div>
            ...
            <ul>
              {results.map((r, i) => (
                <li key={i}>
                  <button onClick={() => { nav(r.to); setOpen(false); }} className="w-full text-left px-5 py-3 hover:bg-[#FAFAF8] flex items-center gap-3 border-t border-slate-100" data-testid={`guest-palette-result-${i}`}>
