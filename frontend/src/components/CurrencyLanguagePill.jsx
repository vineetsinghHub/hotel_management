...
export const CurrencyLanguagePill = () => {
  ...
  return (
    <div ref={ref} className="fixed bottom-20 right-6 z-[60] print:hidden" data-testid="cur-lang-pill-wrap">
      {open && (
        <div
          className="absolute bottom-14 right-0 w-72 rounded-[18px] bg-white border border-slate-200 shadow-[0_20px_50px_rgba(15,23,42,0.15)] p-4 dark-surface"
        ...
        >
              ...
              <button
                ...
                className={`px-2 py-2 rounded-[10px] text-xs font-mono border transition-all ${
                  currency === c.key
                    ...
                    : "bg-[#FAFAF8] text-slate-700 border-transparent hover:border-slate-200"
                }`}
          ...
          <p className="text-eyebrow text-[#C9A227] mt-4">{t("common.language")}</p>
          ...
          <p className="text-eyebrow text-[#C9A227] mt-4">{t("common.theme")}</p>
            ...
            {[
              ...
              <button
                ...
                className={`px-2 py-2 rounded-[10px] text-xs border flex items-center justify-center gap-1.5 transition-all ${
                  theme === it.k
                    ? "bg-[#C9A227] text-slate-900 border-[#C9A227]"
                    : "bg-[#FAFAF8] text-slate-700 border-transparent hover:border-slate-200"
                }`}
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 pl-3 pr-4 py-2.5 rounded-full bg-slate-900 text-white shadow-[0_10px_28px_rgba(15,23,42,0.25)] hover:-translate-y-0.5 transition-all"
        data-testid="cur-lang-pill"
        aria-label="Currency, language and theme"
        aria-expanded={open}
      >
        <span className="w-6 h-6 rounded-full bg-white/10 grid place-items-center text-[10px] font-mono">{curObj.symbol}</span>
        <span className="text-[11px] tracking-widest uppercase">{curObj.key} · {locObj.key.toUpperCase()}</span>
        <i className={`fa-solid fa-chevron-${open ? "down" : "up"} text-[9px]`}></i>
      </button>
    </div>
  );
};
